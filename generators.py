...
from .prod_xs import *
from .fluxes import *
from .target_photon import *
import multiprocessing as multi
from kshell_multipole.multipole_xsec_base import AxionMultipoleXsec
...
class IsotropicAxionFromPrimakoff:
    def __init__(self, photon_rates=[1,1], axion_mass=0.1, axion_coupling=1e-4, gann=1e-4, gaee=1e-4,
                 target_mass=240e3, target_z=90, target_photon_cross=15e-24, target:Material=None, detector_distance=4,
                 detector_length=0.2, detector_area=20):
        self.photon_rates = photon_rates  # per second
        self.axion_mass = axion_mass  # MeV
        self.axion_coupling = axion_coupling  # MeV^-1
        self.gann = gann # MeV^-1
        self.gaee = gaee
        ...
        self.axion_flux = []
        self.axion_flux_det = []
        self.decay_axion_weight = [] # a -> 2 gamma
        self.decay_ep_axion_weight = [] # a -> e+e-
        self.scatter_axion_weight = []
        self.abs_axion_weight = []
        self.deex_photon_energy = [] # deexcitation photon detected
        self.axion_velocity = []
        self.pc = PhotonCoherent(target) # photon-nucleus coherent scattering cross section

    def branching_ratio(self, energy, coupling=1.0):
        # BSM
        # xs = primakoff_sigma(energy, coupling, self.axion_mass, self.target_z)
        xs = primakoff_sigma_massive(energy, self.target_z, self.axion_mass, coupling)

        # SM
        pc = self.pc
        sm_xs = self.target_photon_cross / (100 * METER_BY_MEV) ** 2
        if pc.data is not None:
            sm_xs = pc.xsec(energy) / (100 * METER_BY_MEV) ** 2
    ...
    def simulate_single(self, energy, rate):
        ...
        # a -> e+e-
        me = 0.511
        surv_prob_ep = decay_in_detector_ep = 0
        if self.axion_mass >= 2*me:
            tau_ep = 8*pi / (self.gaee**2 * self.axion_mass * np.sqrt(1 - 4*me**2/self.axion_mass**2)) * axion_boost
            surv_prob_ep =  np.exp(-self.detector_distance / METER_BY_MEV / axion_v / tau_ep)
            decay_in_detector_ep = 1 - np.exp(-self.detector_length / METER_BY_MEV / axion_v / tau_ep)
        ...
        self.axion_flux_det.append(rate * br / (4*pi*self.detector_distance ** 2) * surv_prob)
        self.decay_axion_weight.append(rate * br * surv_prob * decay_in_detector / (4*pi*self.detector_distance ** 2))
        self.decay_ep_axion_weight.append(rate * br * surv_prob_ep * decay_in_detector_ep / (4*pi*self.detector_distance ** 2))
        self.scatter_axion_weight.append(surv_prob * rate * br / (4*pi*self.detector_distance ** 2))
        self.abs_axion_weight.append(surv_prob * rate * br / (4*pi*self.detector_distance ** 2))
    ...
    def decay_events(self, detection_time, threshold):
        ...
        for i in range(len(self.axion_energy)):
            if self.axion_energy[i] < threshold:
                self.decay_axion_weight[i] = 0.0
                self.decay_ep_axion_weight[i] = 0.0
                continue

            self.decay_axion_weight[i] *= detection_time * self.det_area
            self.decay_ep_axion_weight[i] *= detection_time * self.det_area
            res += self.decay_axion_weight[i]

        return res

    def scatter_events(self, detector_number, detector_zs, detection_time, threshold):
        res = 0
        for i in range(len(self.axion_energy)):
            if self.axion_energy[i] < threshold:
                self.scatter_axion_weight[i] = 0.0
            ...
            xs = 0
            for detector_z in detector_zs: # sum over all nucleus
                # xs += iprimakoff_sigma(self.axion_energy[i], self.axion_coupling, self.axion_mass, detector_z)
                xs += iprimakoff_sigma_massive(self.axion_energy[i], detector_z, self.axion_mass, self.axion_coupling)
    ...
    def absorption_events_multipole(self, detector_number, detection_time, threshold, axion_mx: AxionMultipoleXsec,
        ...
        for i in range(len(self.axion_energy)):
            ...
            ea, ma = self.axion_energy[i], self.axion_mass
            the_delta_fun = gaussian(ea, mu=ea, sigma=1e-2)
            xs = axion_mx.get(ea, ma, self.gann, interaction) * the_delta_fun # cross section [MeV^-2]
            photo_energies.append(ea)

            self.abs_axion_weight[i] = wgt * xs * detection_time * detector_number * METER_BY_MEV ** 2
            res += self.abs_axion_weight[i]
    ...
    def propagate(self): # WARNING: deprecate, not being used
        g = self.axion_coupling
        e_a = np.array(self.axion_energy)
        wgt = np.array(self.axion_flux)
